...
```bash
# Build image
docker build -t rahzom-test-image .agents/skills/sandbox-linux-init/

# Create container (use wrapper script)
.agents/skills/sandbox-linux-init/docker-run.sh
```
...
```bash
.agents/skills/sandbox-linux-init/docker-exec.sh tmux kill-session -t main 2>/dev/null || true
.agents/skills/sandbox-linux-init/docker-exec.sh tmux new-session -d -s main -x 120 -y 40
```

### 6. Build rahzom

```bash
.agents/skills/sandbox-linux-init/docker-exec.sh cargo build --release
```

### 7. Verify setup

```bash
.agents/skills/sandbox-linux-init/docker-exec.sh ls -la /app/rahzom/target/release/rahzom
.agents/skills/sandbox-linux-init/docker-exec.sh tmux list-sessions
```
